...
/// A regular expression to evaluate comparison operations. [Interactive example](https://regex101.com/r/j2CtoW/1)
pub const RE_CMP: &str = r#"(?m)(?<left>[^\s]+)\s+(?P<op>[^\s]+)\s+(?P<right>.+)"#;
